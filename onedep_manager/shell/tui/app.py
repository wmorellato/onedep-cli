...
class OneDepTuiApp(FilesCommands, NavigationCommands, App):
    ...
    def on_input_submitted(self, event: HistoryInput.Submitted) -> None:
        ...
        try:
            ...
            if action == "entry":
                ...
                self.do_cd(rest)
            elif action in CD_MNEMONICS:
                self.do_cd_mnemonic(action, rest)
            elif action == "cdwfi":
                self.do_cdwfi(rest)
            elif action == "ls":
                self.dispatch(["list"] + rest)
            elif action == "ff":
                self.dispatch(["find"] + rest)
            elif action == "files":
                self.dispatch(rest)
            elif action == "scripts":
    ...
    def _do_help(self) -> None:
        rows = [
            ...
            ["cd <repo> [id]", "Cd into a repository path for an entry"],
            [", ".join(sorted(CD_MNEMONICS)) + " [id]", "Shortcuts for cd <repo> [id]"],
            ["cdwfi [entry_id] <wfinst_id>", "Cd into a workflow instance directory"],
            ["files find|list|hash|info [args...]", "Find/list/hash/info on wwPDB files"],
            ["ls / ff [args...]", "Aliases for files list / files find"],
