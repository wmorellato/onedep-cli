...
@pytest.mark.asyncio
async def test_cd_changes_directory_and_sets_entry(tmp_path, monkeypatch):
    ...
    async with app.run_test() as pilot:
        ...
        input_widget = app.query_one("#cmdline")
        input_widget.value = "cd archive D_800000"
        await pilot.press("enter")
...
@pytest.mark.asyncio
async def test_cdd_mnemonic_changes_directory(tmp_path, monkeypatch):
    monkeypatch.chdir(tmp_path)
    ...
    async with app.run_test() as pilot:
        ...
        input_widget = app.query_one("#cmdline")
        input_widget.value = "cdd D_800000"
        await pilot.press("enter")
...
@pytest.mark.asyncio
async def test_cd_unknown_repo_reports_clean_error(tmp_path, monkeypatch):
    monkeypatch.chdir(tmp_path)
    app = _app(tmp_path)
    async with app.run_test() as pilot:
        app.printer = RecordingPrinter()
        ...
        input_widget = app.query_one("#cmdline")
        input_widget.value = "cd not-a-repo D_800000"
        await pilot.press("enter")

        assert any("Unknown repository" in msg for msg in app.printer.error_calls)


@pytest.mark.asyncio
async def test_cdwfi_uses_current_entry(tmp_path, monkeypatch):
    monkeypatch.chdir(tmp_path)
    ...
    async with app.run_test() as pilot:
        ...
        input_widget = app.query_one("#cmdline")
        input_widget.value = "cdwfi W_001"
        await pilot.press("enter")
...
@pytest.mark.asyncio
async def test_ls_aliases_files_list(tmp_path, monkeypatch):
    monkeypatch.chdir(tmp_path)
    ...
    async with app.run_test() as pilot:
        ...
        input_widget = app.query_one("#cmdline")
        input_widget.value = "ls --type model"
        await pilot.press("enter")
...
@pytest.mark.asyncio
async def test_ff_aliases_files_find(tmp_path, monkeypatch):
    monkeypatch.chdir(tmp_path)
    (tmp_path / "D_800000_model_P1.cif.V1").write_text("contents")
    ...
    async with app.run_test() as pilot:
        ...
        input_widget = app.query_one("#cmdline")
        input_widget.value = "ff --type model"
        await pilot.press("enter")
